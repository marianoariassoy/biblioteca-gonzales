...
if (isset($_POST['form'])) {
  ...
  if ($action == 'Agregar') {
    ...
    $sql = 'INSERT INTO ' . $table . ' 
		(`type`, `title`, `text`,   `top`, `image`, `url`, `file`, `video` ) 
		VALUES 
		("' . $sec  . '", "' . $title . '", "' . $text . '" ,  "' . $top . '",  "' . $image . '",  "' . $url . '",  "' . $file . '",  "' . $video . '" )';
...
if (isset($_GET['add'])) {
  ...
  $action = 'Agregar';
  $title_sec = "Agregar Entrada";
} else if (isset($_GET['edit'])) {

  $action = 'Modificar';
  $title_sec = "Modificar Entrada";
  $id_select = $_GET['i'];
