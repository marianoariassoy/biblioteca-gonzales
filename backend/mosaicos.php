...
if (isset($_POST['form'])) {
  ...
  if ($action == 'Agregar') {
    ...
    $sql = 'INSERT INTO ' . $table . ' 
		(`type`, `title`, `text2`, `text`, `image`, `top` ) 
		VALUES 
		("' . $sec  . '", "' . $title . '", "' . $text2 . '", "' . $text . '", "' . $image . '",  "' . $top . '")';
    ...
    $sql = 'UPDATE ' . $table . ' 
    SET 
    `title` = "' . $title  . '", `text2` = "' . $text2 . '", `text` = "' . $text . '", `top` = "' . $top . '" ';
...
if (isset($_GET['add'])) {
  ...
  $action = 'Agregar';
  $title_sec = "Agregar Mosaico";
} else if (isset($_GET['edit'])) {

  $action = 'Modificar';
  $title_sec = "Modificar Mosaico";
  $id_select = $_GET['i'];
