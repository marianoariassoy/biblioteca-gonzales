...
// $sql = 'UPDATE items SET `top` = "20" WHERE `id` = "9";';
...
if (isset($_GET['add'])) {
  ...
  $action = 'Agregar';
  $title_sec = "Agregar Archivo";
} else if (isset($_GET['edit'])) {

  $action = 'Modificar';
  $title_sec = "Modificar Archivo";
  $id_select = $_GET['i'];
  ...
  $title_sec = "Archivos";
  $sql = "SELECT * FROM $table WHERE type = $sec ORDER BY category ASC, top DESC";
  $statement = $pdo->prepare($sql);
  $statement->execute();
  $results = $statement->fetchAll();
  $total = count($results);
  $order_list = array('top' => 'Prioridad', 'title' => 'Título', 'category' => 'Categoría');
}
require("includes/head.php");

$categories = array('Catálogo Biblioteca', 'Catálogo Videoteca', 'Catálogo HG');
        ...
        <form method="POST" name="form" enctype="multipart/form-data">
          <div class="row mb-5">
            ...
            <!-- <div class="col col-12 col-md-6 mb-3">
              <label class="form-label">Cateogoría</label>

              <select class="form-select bg-transparent text-primary border-primary rounded-0" name="category">
                <?php
                foreach ($categories as $indice => $value) {
                  if ($category == $indice) $aux = "selected";
                  else $aux = "";
        ...
        ?>
          <table class="table table-dark table-hover">
            ...
            <tbody>
              ...
              <tr>
                <td valign="middle" class="text-primary">
                  <a href="sobre-archivos"><strong>Sobre Archivos</strong></a>
                </td>
                <td valign="middle" align="right">
                  <a href="sobre-archivos" class="circle bg-transparent"><i class="bi bi-file-earmark-text"></i></a>
                </td>
              </tr>
            </tbody>
          </table>
