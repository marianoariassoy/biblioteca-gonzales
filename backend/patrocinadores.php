...
if (isset($_POST['form'])) {
  ...
  $data = "userfile1";
  if ($_FILES[$data]['name']) {
    $image = "gonzalez" . GenerateString(10) . ".png";
    copy($_FILES[$data]['tmp_name'], "./images/" . $image);
  } else $image = "";

  if ($action == 'Agregar') {

    $sql = 'INSERT INTO ' . $table . ' (`type`, `image`, `url`, `title`, `top`) 
    VALUES ("' . $sec . '", "' . $image . '", "' . $text . '", "' . $title . '", "' . $top . '")';
...
if (isset($_GET['add'])) {
  $action = 'Agregar';
  $title_sec = "Agregar Patrocinador";
} else if (isset($_GET['edit'])) {
  $action = 'Modificar';
  $title_sec = "Modificar Patrocinador";
  $id_select = $_GET['i'];
        ...
        <form method="POST" name="form" enctype="multipart/form-data">
          <div class="row mb-5">
            <div class="col col-12 mb-3">
              ...
              <div class="row text-secondary">Recomendado: 300 px de ancho en formato png transparente.</div>
              <?php if (isset($results['image'])) if ($results['image'])  echo '<img src="./images/' . $results['image'] . '" class="mt-3 mb-3" height="100" /> '; ?>
            </div>
            <div class="col col-12 mb-3">
            ...
            <tbody>
              ...
              ?>
                <tr>
                  <td valign="middle">
                    <?php
                    if ($image) echo '<img src="./images/' . $image . '" height="50" />';
                    ?>
                  </td>
