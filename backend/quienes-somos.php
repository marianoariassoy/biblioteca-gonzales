...
$file_main = "quienes-somos";
$table = "items";
$sec = 3;

// $sql = 'UPDATE items SET `top` = "10" WHERE `id` = "47";';
...
if (isset($_POST['form'])) {
  ...
  if ($action == 'Agregar') {
    ...
    $sql = 'INSERT INTO ' . $table . ' 
		(`type`, `title`, `text`, `image`, `url`, `top` ) 
		VALUES 
		("' . $sec  . '", "' . $title . '", "' . $text . '", "' . $image . '"
    , "' . $url . '",  "' . $top . '" )';
    $statement = $pdo->prepare($sql);
    $statement->execute();
    ...
    $sql = 'UPDATE ' . $table . ' 
    SET 
    `title` = "' . $title  . '", `text` = "' . $text . '", `url` = "' . $url . '", `top` = "' . $top . '"';
...
if (isset($_GET['add'])) {
  ...
  $action = 'Agregar';
  $title_sec = "Agregar Quienes Somos";
} else if (isset($_GET['edit'])) {

  $action = 'Modificar';
  $title_sec = "Modificar Quienes Somos";
  $id_select = $_GET['i'];
  ...
  <main>
    ...
    <?php } else { ?>
      ...
      <section class="p-3 p-lg-5 p-md-4 text-white">
        <div class="row mb-4">
          <div class="col-12">
            <h2> <strong><?php echo $title_sec ?></strong> (<?php echo $total + 1 ?>)</h2>
          </div>
        ...
        ?>
          <table class="table table-dark table-hover">
            ...
            <tbody>
              ...
              <!-- <tr>
                <td valign="middle" class="text-primary">
                  <a href="decalogo"><strong>Decálogo</strong></a>
                </td>
                <td valign="middle" align="right">
                  <a href="decalogo" class="circle bg-transparent"><i class="bi bi-file-earmark-text"></i></a>
                </td>
              </tr> -->
              <tr>
                <td valign="middle" class="text-primary">
                  <a href="donaciones"><strong>Donaciones</strong></a>
                </td>
                <td valign="middle" align="right">
                  <a href="donaciones" class="circle bg-transparent"><i class="bi bi-file-earmark-text"></i></a>
                </td>
              </tr>
            </tbody>
          </table>
